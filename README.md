...
AI-native development means writing specifications that an AI agent turns into tested, committed code — rather than writing every line yourself. The workflow in this cookbook gives you the structure to do this reliably: plan first with Spec-Kit, execute with Superpowers, and never let the agent guess what you want.

**Zero to first feature — three steps:**

1. Read [ONBOARDING.md](./ONBOARDING.md) (~30 min) — plain-language walkthrough for any background
2. Install Spec-Kit + Superpowers (15 min) — see [Installation in SUPERPOWERS_SPECKIT_COOKBOOK.md](./SUPERPOWERS_SPECKIT_COOKBOOK.md#2-installation)
3. Run `/speckit.specify` on a real task — follow the brownfield or greenfield scenario in ONBOARDING.md

**If you are coming from traditional development, think of it this way:**

| What you know | What replaces it here |
|---|---|
| Product Requirements Document (PRD) | `spec.md` — generated by `/speckit.specify` + `/speckit.clarify` |
| Sprint planning | `/speckit.plan` |
| Story tickets / Jira cards | `tasks.md` — generated by `/speckit.tasks` |
| Code review | Superpowers `requesting-code-review` skill |
| Retrospective / post-mortem | `/speckit.analyze` (optional) |

**The one concept to internalize before you start:** `tasks.md` is the handoff artifact. Spec-Kit produces it. Superpowers executes it. Do not skip the handoff message between the two tools.

-> Full beginner guide: [ONBOARDING.md](./ONBOARDING.md)
